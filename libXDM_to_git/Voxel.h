...
public:
  ...
  //--------------------------------------
  //  SetLeftVertex  -- define the voxel by the left vertex,
  //                    as per description of Bob's MIC format
  //--------------------------------------
  void SetLeftVertex( const SVector3 & v,
                      Float _fSideLength,
                      int VoxelOrientation )
  {
    Float fX = v.m_fX;
    Float fY = v.m_fY;
    Float fZ = v.m_fZ;
    
    if ( VoxelOrientation == UP  ) 
    {
      // Winding order, counter clockwise
      pVertex[0].Set( fX,                 fY, fZ );
      pVertex[1].Set( fX + _fSideLength, fY, fZ );
      pVertex[2].Set( fX + _fSideLength / (Float) 2.0 , fY + _fSideLength / (Float) 2.0 * sqrt( (Float) 3.0 ) , fZ);
      bVoxelPointsUp = true;
    }
    else
    {
      // Winding order, counter clockwise
      pVertex[0].Set( fX,                 fY, fZ );
      pVertex[1].Set( fX + _fSideLength / (Float) 2.0 , fY - _fSideLength / (Float) 2.0 * sqrt( (Float) 3.0 ) , fZ);
      pVertex[2].Set( fX + _fSideLength, fY, fZ );
      bVoxelPointsUp = false;
    }
    fSideLength = _fSideLength;
  }
