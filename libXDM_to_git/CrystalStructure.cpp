  ...
  for( int i = 0; i < 3; i++ )
    {
      for( int j = 0; j < 3; j++ )
	{
	  // see pg 118 of DeGraef "Structure of Materials"                                                                        
	  oRecpLatMetricTensor.m[i][j] = Dot( oReciprocalVector[i] / ( 2 * PI), oReciprocalVector[j] / ( 2 * PI ) );
	}
    }
...
CRecpVector CUnitCell::GetReciprocalVector( int h, int k, int l ) const
{
  ...
  oRefRecipVector.d = static_cast<Float>(1) / sqrt( oRefRecipVector.d );    
