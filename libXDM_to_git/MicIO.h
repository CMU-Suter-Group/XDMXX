...
//       return false;
    ...
    int NumTokens = 0;
    if( ( NumTokens = GeneralLib::TokenizeLine( Tokens, InStream, " \t\n" ) ) != 1 )
    {
    ...
    oVoxelList.clear();
    fInitialSideLength = atof(Tokens[0].c_str());

    int LineNumber = 1;
    while( TokenizeLine( Tokens, InStream, " \t\n" ) >= 9 )
    {
      if( Tokens.size() < 9 )
      {
      ...
      {
        DEBUG_ALERT( Tokens.size() >= 10, "MicFileBase::Read:  Warning!  Old format MIC file with only 9 columns\n" );
        VoxelType v;
        v.nGeneration = atoi( Tokens[4].c_str() );
        v.fSideLength = fInitialSideLength / pow( 2,  v.nGeneration ) ;
        v.nPhase = atoi( Tokens[5].c_str() );

        Float fX, fY, fZ;

        fX = atof( Tokens[0].c_str() );
        fY = atof( Tokens[1].c_str() );
        fZ = atof( Tokens[2].c_str() );

        if ( atoi( Tokens[3].c_str() ) == UP  ) 
        {
        ...
        oOrientation.Set(  DEGREE_TO_RADIAN( atof(Tokens[6].c_str()) ),
                           DEGREE_TO_RADIAN( atof(Tokens[7].c_str()) ),
                           DEGREE_TO_RADIAN( atof(Tokens[8].c_str()) ) );
        ...
        v.oOrientMatrix.BuildActiveEulerMatrix( oOrientation.m_fX,
                                                ...
                                                oOrientation.m_fZ );
        if( Tokens.size() > 9 )
          v.fConfidence = atof( Tokens[9].c_str() );
        else
          v.fConfidence = 0;
        
        if( Tokens.size() == 19 )   //  Defining our strain tensor to be ( I)
        {
          v.fCost                = atof( Tokens[10].c_str() );
          v.fPixelOverlapRatio   = atof( Tokens[11].c_str() );
          //  column Tokens[12] is quality, which doesn't exist in SVoxel
          v.oDeformation.m[0][0] = atof( Tokens[13].c_str() );
          v.oDeformation.m[1][1] = atof( Tokens[14].c_str() );
          v.oDeformation.m[2][2] = atof( Tokens[15].c_str() );
        
          v.oDeformation.m[0][1] = v.oDeformation.m[1][0] = atof( Tokens[16].c_str() );
          v.oDeformation.m[1][2] = v.oDeformation.m[2][1] = atof( Tokens[17].c_str() );
          v.oDeformation.m[0][2] = v.oDeformation.m[2][0] = atof( Tokens[18].c_str() );
        }
        else
        {
